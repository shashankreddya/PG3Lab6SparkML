...
object SparkRandomForest {
  def main(args: Array[String]) {
    ...
    val sparkConf =new SparkConf().setMaster("local[*]").setAppName("SparkRandomForest").set("spark.driver.memory", "4g")
    ...
    val maxBins = 32
    val numTrees=3
    val featureSubsetStartegy="auto"
    val model = RandomForest.trainClassifier(X_train, numClasses, categoricalFeaturesInfo,numTrees,featureSubsetStartegy,impurity, maxDepth, maxBins)
    ...
    println(accuracy)
    evaluateModel(predictionAndLabel,"Random Forest Results")
